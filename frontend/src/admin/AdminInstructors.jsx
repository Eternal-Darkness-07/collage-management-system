...
const AdminInstructors = () => {
  ...
  const onSubmitInstructor = async (e) => {
    ...
    if (
      ...
      !instructorData.department_id
    )
      return

    try {
      if (editing) {
        await axios.put(`${baseURL}/api/instructors/admin/${editingId}`, instructorData, {
          withCredentials: true,
        })
      } else {
        await axios.post(`${baseURL}/api/instructors/admin/add`, instructorData, {
          withCredentials: true,
        })
      }
        ...
        <CTableBody>
          {instructors.map((instructor) => {
            const department = departments.find(
              (d) => d.department_id === instructor.department_id,
            )?.department_name
            return (
